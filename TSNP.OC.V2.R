## generate the OC of TSNP design
## equipped with early stopping rules for toxicity and efficacy based on p-values


## Generate the joint toxicity-efficacy probability matrix based on the cross ratio model
...
########################################################################################################################################################################################################
##### TSNP.OC generates the operating characteristic of the TSNP design ################################################################################################################################
...
##### tau: the margin of toxicity used in the first stage ##############################################################################################################################################
##### m: if m patients have been treated at the current dose, swith the trial to the second stage ######################################################################################################
##### phi.t: the maximum tolerable toxicity rate #######################################################################################################################################################
##### c.t: cut-off p-value for early stopping for toxicity #############################################################################################################################################
##### phi.e: the minimum tolerable efficacy rate #######################################################################################################################################################
##### c.e: cut-off p-value for early stopping for futility #############################################################################################################################################
##### omega: the vector of utility scores for T=0 and E=0; T=0 and E=1; T=1 and E=0; T=1 and E=1 ########################################################################################################
...
# 
# 
 pi=crossratio(ttox=c(0.1,0.2,0.4,0.45,0.5), teff=c(0.1,0.4,0.6,0.65,0.7), gamma=c(1,2,2,3,3))
# 
 TSNP.OC(pi,cohortsize=2,ncohort=20,tau=0.1,m=8,phi.t=0.3, c.t=0.1, phi.e=0.4, c.e=0, omega=c(25,100,0,50), start=1,ntrial=10000)
# 
